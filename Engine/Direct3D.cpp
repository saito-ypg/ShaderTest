...
HRESULT Direct3D::Initialize(int winW, int winH, HWND hWnd)
{
	...
	//いろいろな設定項目をまとめた構造体
		...
		if (FAILED(hr))
		{
			MessageBox(nullptr, "デバイスもしくはスワップチェインの作成に失敗しました", "エラー", MB_OK);
			return hr;
		}
		///////////////////////////レンダーターゲットビュー作成///////////////////////////////
...
HRESULT Direct3D::InitShader()
{
	...
	hr = InitShaderToonOutline();
	if (FAILED(hr))
	{
		return hr;
	}
	hr = InitShaderNormalMap();
	if (FAILED(hr))
	{
		return hr;
	}
...
HRESULT Direct3D::InitShader2D()
{
	...
	{// 頂点シェーダの作成（コンパイル）		
		...
		hr = pDevice_->CreateInputLayout(layout, sizeof(layout) / sizeof(layout[0]), pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_2D].pVertexLayout_));
		if (FAILED(hr))
		{
			MessageBox(nullptr, "頂点インプットレイアウトの作成に失敗しました", "エラー", MB_OK);
			return hr;
...
HRESULT Direct3D::InitShader3DEffected()
{
	// 頂点シェーダの作成（コンパイル）		
	ID3DBlob* pCompileVS = nullptr;
	D3DCompileFromFile(L"Effect3D.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
	assert(pCompileVS != nullptr);

	HRESULT	hr = pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &(shaderBundle[SHADER_3DE].pVertexShader_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "頂点シェーダーの作成に失敗しました", "エラー", MB_OK);
		return hr;
	...
	hr = pDevice_->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_3DE].pVertexLayout_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "頂点インプットレイアウトの作成に失敗しました", "エラー", MB_OK);
		SAFE_RELEASE(pCompileVS);
	...
	//ピクセルシェーダの作成（コンパイル）
	ID3DBlob* pCompilePS = nullptr;
	hr = D3DCompileFromFile(L"Effect3D.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	assert(pCompilePS != nullptr);

	hr = pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &(shaderBundle[SHADER_3DE].pPixelShader_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "ピクセルシェーダの作成に失敗しました", "エラー", MB_OK);
		SAFE_RELEASE(pCompilePS);
	...
	rdc.FillMode = D3D11_FILL_SOLID;
	rdc.FrontCounterClockwise = FALSE;
	hr = pDevice_->CreateRasterizerState(&rdc, &(shaderBundle[SHADER_3DE].pRasterizerState_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "ラスタライザの作成に失敗しました", "エラー", MB_OK);
		return hr;
...
HRESULT Direct3D::InitShader3DToon()
{
	// 頂点シェーダの作成（コンパイル）		
	ID3DBlob* pCompileVS = nullptr;
	D3DCompileFromFile(L"Toon3D.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
	assert(pCompileVS != nullptr);

	HRESULT	hr = pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &(shaderBundle[SHADER_3DToon].pVertexShader_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "頂点シェーダーの作成に失敗しました", "エラー", MB_OK);
		return hr;
	...
	hr = pDevice_->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_3DToon].pVertexLayout_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "頂点インプットレイアウトの作成に失敗しました", "エラー", MB_OK);
		SAFE_RELEASE(pCompileVS);
	...
	//ピクセルシェーダの作成（コンパイル）
	ID3DBlob* pCompilePS = nullptr;
	hr = D3DCompileFromFile(L"Toon3D.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	assert(pCompilePS != nullptr);

	hr = pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &(shaderBundle[SHADER_3DToon].pPixelShader_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "ピクセルシェーダの作成に失敗しました", "エラー", MB_OK);
		SAFE_RELEASE(pCompilePS);
	...
	rdc.FillMode = D3D11_FILL_SOLID;
	rdc.FrontCounterClockwise = FALSE;
	hr = pDevice_->CreateRasterizerState(&rdc, &(shaderBundle[SHADER_3DToon].pRasterizerState_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "ラスタライザの作成に失敗しました", "エラー", MB_OK);
		return hr;
...
HRESULT Direct3D::InitShaderToonOutline()
{
	...
	HRESULT	hr = pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &(shaderBundle[SHADER_ToonOutline].pVertexShader_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "頂点シェーダーの作成に失敗しました", "エラー", MB_OK);
		return hr;
	...
	hr = pDevice_->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_ToonOutline].pVertexLayout_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "頂点インプットレイアウトの作成に失敗しました", "エラー", MB_OK);
		SAFE_RELEASE(pCompileVS);
	...
	//ピクセルシェーダの作成（コンパイル）
	ID3DBlob* pCompilePS = nullptr;
	hr = D3DCompileFromFile(L"ToonOutline.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	assert(pCompilePS != nullptr);
...
HRESULT Direct3D::InitShaderNormalMap()
{
	// 頂点シェーダの作成（コンパイル）		
	ID3DBlob* pCompileVS = nullptr;
	...
	HRESULT	hr = pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &(shaderBundle[SHADER_NORMALMAP].pVertexShader_));
	if (FAILED(hr))
	{
		MessageBox(nullptr, "頂点シェーダーの作成に失敗しました", "エラー", MB_OK);
		return hr;
